...
EXPERIMENT_NAME = "q_learning_bucketed_with_phase_wait_small_vehicle"
...
def build_phase_to_bucket() -> dict[int, int]:
...
def bucket_phase(phase_id: int) -> int:
    phase_to_bucket = build_phase_to_bucket()
    if phase_id not in phase_to_bucket:
        raise ValueError(
            f"Unexpected phase_id {phase_id}; expected one of {list(phase_to_bucket)}"
        )
    return phase_to_bucket[phase_id]


def discretize_state(obs: np.ndarray, phase_id: int) -> tuple[int, int, int, int, int]:
    directional_totals = aggregate_by_direction(obs)
    return (
        bucket_queue_length(float(directional_totals[0])),
        bucket_queue_length(float(directional_totals[1])),
        bucket_queue_length(float(directional_totals[2])),
        bucket_queue_length(float(directional_totals[3])),
        bucket_phase(int(phase_id)),
    )


def reward_wait_small_vehicle(info: dict, prev_info: dict | None) -> float:
    return -(float(info["total_waiting"]) + 0.05 * float(info["vehicle_count"]))


def epsilon_greedy_action(
    q_table: DefaultDict[tuple[int, int, int, int, int], np.ndarray],
...
def train_q_learning(
...
) -> tuple[DefaultDict[tuple[int, int, int, int, int], np.ndarray], list[dict]]:
    ...
    for episode in range(episodes):
        ...
        obs, info = env.reset()
        state = discretize_state(obs, info["phase_id"])
        ...
        while not done:
            ...
            reward = reward_wait_small_vehicle(next_info, info)
            done = terminated or truncated

            next_state = discretize_state(next_obs, next_info["phase_id"])
...
def run_q_learning_episode(
    ...
    while not done:
        ...
        reward = reward_wait_small_vehicle(info, prev_info)
        state = discretize_state(obs, info["phase_id"])
        total_reward += reward
        ...
        trace_rows.append(
            {
                ...
                "state_north_bucket": state[0],
                "state_east_bucket": state[1],
                "state_south_bucket": state[2],
                "state_west_bucket": state[3],
                "state_phase_bucket": state[4],
                "reward": reward,
...
def main() -> None:
    ...
    for dataset_dir in dataset_dirs:
        ...
        print("\n" + "=" * 60)
        print("Training Q-learning with bucketed queues, with phase, wait+small-vehicle reward...")
