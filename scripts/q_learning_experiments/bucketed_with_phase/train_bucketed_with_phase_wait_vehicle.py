...
EXPERIMENT_NAME = "q_learning_bucketed_with_phase_wait_vehicle"
...
def reward_wait_vehicle(info: dict, prev_info: dict | None) -> float:
    return -(float(info["total_waiting"]) + 0.1 * float(info["vehicle_count"]))
...
def train_q_learning(
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            reward = reward_wait_vehicle(next_info, info)
...
def run_q_learning_episode(
    ...
    while not done:
        ...
        reward = reward_wait_vehicle(info, prev_info)
        state = discretize_state(obs, info["phase_id"])
        total_reward += reward
...
def main() -> None:
    ...
    for dataset_dir in dataset_dirs:
        ...
        print("\n" + "=" * 60)
        print("Training Q-learning with bucketed queues, with phase, wait+vehicle reward...")
