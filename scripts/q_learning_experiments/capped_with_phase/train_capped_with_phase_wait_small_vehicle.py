...
EXPERIMENT_NAME = "q_learning_capped_with_phase_wait_small_vehicle"
...
def build_phase_to_index() -> dict[int, int]:
...
def phase_to_index(phase_id: int) -> int:
    phase_to_idx = build_phase_to_index()
    if phase_id not in phase_to_idx:
        raise ValueError(
            f"Unexpected phase_id {phase_id}; expected one of {list(phase_to_idx)}"
        )
    return phase_to_idx[phase_id]
...
def discretize_state(
    ...
    phase_idx = phase_to_index(int(info["phase_id"]))
    return tuple(int(x) for x in capped) + (phase_idx,)
...
def train_q_learning(
    ...
    for episode in range(episodes):
        ...
        obs, info = env.reset()
        state = discretize_state(obs, info)
        ...
        while not done:
            ...
            next_state = discretize_state(next_obs, next_info)
...
def main() -> None:
    ...
    for dataset_dir in dataset_dirs:
        ...
        print("\n" + "=" * 60)
        print("Training with capped counts + phase, wait + small vehicle penalty...")
